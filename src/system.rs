...
impl fmt::Display for Model {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            ...
            Model::RaspberryPiComputeModule0 => write!(f, "Raspberry Pi Compute Module 0"),
...
fn parse_proc_cpuinfo() -> Result<Model> {
    ...
    let model = if (revision.len() == 4) || (revision.len() == 8) {
        ...
        match revision_type {
            ...
            0x1b => Model::RaspberryPiComputeModule0,
            _ => return Err(Error::UnknownModel),
        }
    } else {
...
fn parse_base_compatible() -> Result<Model> {
    ...
    for comp_id in base_compatible.split('\0') {
        let model = match comp_id {
            ...
            "raspberrypi,0-compute-module" => Model::RaspberryPiComputeModule0,
...
fn parse_base_model() -> Result<Model> {
    ...
    let model = match &base_model[..] {
        ...
        "Raspberry Pi Compute Module 0" => Model::RaspberryPiComputeModule0,
...
impl DeviceInfo {
    ...
    pub fn new() -> Result<DeviceInfo> {
        ...
        match model {
            ...
            Model::RaspberryPi3B | Model::RaspberryPiComputeModule3 | Model::RaspberryPiComputeModule0 | Model::RaspberryPiZero2W => {
                Ok(DeviceInfo {
                    model,
                    soc: SoC::Bcm2837A1,
                    peripheral_base: PERIPHERAL_BASE_RPI2,
